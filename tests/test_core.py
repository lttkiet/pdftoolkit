...
def test_insert_text_morph_rotation():
    doc = fitz.open()
    page = doc.new_page()
    ...
    assert "Rotated" in page.get_text()
    doc.close()


def test_insert_image_alpha_stream_blends():
    import io
    ...
    pix = page.get_pixmap(clip=fitz.Rect(30, 30, 31, 31))
    r, g, b = pix.pixel(0, 0)
    assert r == 255
    assert 100 < g < 160
    assert 100 < b < 160
